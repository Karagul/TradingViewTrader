...
from python_telegram_bot_master.telegram import Bot, TelegramError
from python_telegram_bot_master.telegram.ext import Dispatcher, JobQueue
from python_telegram_bot_master.telegram.error import Unauthorized, InvalidToken, RetryAfter, TimedOut
from python_telegram_bot_master.telegram.utils.helpers import get_signal_name
from python_telegram_bot_master.telegram.utils.request import Request
from python_telegram_bot_master.telegram.utils.webhookhandler import (WebhookServer, WebhookAppClass)
...
class Updater(object):
    ...
    receive the updates from python_telegram_bot_master.telegram and to deliver them to said dispatcher. It also runs in a
    separate thread, so the user can interact with the bot, for example on the command line. The
    dispatcher supports handlers for different kinds of data: Updates from python_telegram_bot_master.telegram, basic text
    ...
    def start_polling(self,
        ...
        """Starts polling updates from python_telegram_bot_master.telegram.
    ...
    def _start_polling(self, poll_interval, timeout, read_latency, bootstrap_retries, clean,
        ...
        # updates from python_telegram_bot_master.telegram and inserts them in the update queue of the
        # Dispatcher.
    ...
    def _bootstrap(self, max_retries, clean, webhook_url, allowed_updates, cert=None,
        ...
        def bootstrap_clean_updates():
            self.logger.debug('Cleaning updates from python_telegram_bot_master.telegram server')
            updates = self.bot.get_updates()
            while updates:
